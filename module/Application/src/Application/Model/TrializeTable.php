...
class TrializeTable extends AbstractTableGateway {

 protected $table = 'trialize';

 public function __construct(Adapter $adapter) {
  $this->adapter = $adapter;
  $this->resultSetPrototype = new ResultSet();
  $this->resultSetPrototype->setArrayObjectPrototype(new Trialize());
  $this->initialize();
 }
 ...
 public function launched($package_name){
	 
	 $this->update(['launch_count' => new \Zend\Db\Sql\Expression('launch_count + 1'), 'last_launched' => Date('Y-m-d h:i:s')], ['package_name' => $package_name]);
 ...
 public function search($param){
    ...
    $object = get_object_vars(new Trialize());
